  ...
  return (
    <>
      <div className="container mt-5 border border-2 text-center w-50">
          ...
          <TextField id="outlined-basic" label="Discount Value" className='mb-3 text-center'value={discount} onChange={(e) => setDiscount(Number(e.target.value))} />
          </div>
          <div className="mt-3">
            <h1 className='text-danger'>&#8377; {finalAmount.toFixed(2)}</h1>
          </div>
          <div className="btn border border-2 mt-5 mb-3">
            <Button onClick={findAmount} className=''  type="submit" style={{color:"brown"}}>Calculate</Button>
          </div>
      </div>
    </>
  )
